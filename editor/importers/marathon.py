...
def map_polygon_to_face(polygon):
    return {
        'floorz': polygon.floor_height,
        'ceilingz': polygon.ceiling_height,
        'floor_tex': Texture(polygon.floor_texture),
        'ceiling_tex': Texture(polygon.ceiling_texture),
    }


class MarathonImporterDialog(ImporterDialogBase):
    ...
    def get_options(self):
        return {'map_index': self.combo.current_text()}


class MarathonImporter(ImporterBase):

    format = 'Marathon (*.sceA)'
    ...
    def create_dialog(self) -> QDialog:
        return MarathonImporterDialog(list(self._wad.maps))

    def run(self, graph: Graph, **kwargs):

        m = self._wad.maps[kwargs['map_index']]
        ...
        for i, polygon in enumerate(m.polygons):  # [0:1024]):
            ...
            face_attrs = map_polygon_to_face(polygon)
